import {Mutation, GeneticProfile, GeneGeneticData} from "../../api/generated/CBioPortalAPI";
...
export default class accessors {
    ...
    constructor(public geneticProfiles: GeneticProfile[]) {
    ...
    public alterationType(geneticProfileId:string) {
        const ret = _.find(this.geneticProfiles, (profile: GeneticProfile) => {
            return profile.geneticProfileId === geneticProfileId;
        });
        return (ret) ? ret.geneticAlterationType : undefined;
    }

    public cna(d: GeneGeneticData) {
        if (this.alterationType(d.geneticProfileId) === 'COPY_NUMBER_ALTERATION') {
    ...
    public exp(d: GeneGeneticData) {
        if (this.alterationType(d.geneticProfileId) === 'MRNA_EXPRESSION') {
    ...
    public prot(d: GeneGeneticData) {
        if (this.alterationType(d.geneticProfileId) === 'PROTEIN_LEVEL') {
            return parseFloat(d.value);
        } else {
            return null;
        }
