...
export const SampleListCategoryMap = {
...
//
    ...
    readonly givenSampleOrder = remoteData<Sample[]>({
        await: ()=>[
            this.samples,
            this._sampleSpecification
        ],
        invoke: async()=>{
            // for now, just assume we won't mix sample lists and samples in the specification
            if (this._sampleSpecification.result!.find(x=>!x.sampleId)) {
            ...
            const studyToSampleToIndex:{[studyId:string]:{[sampleId:string]:number}} =
                _.reduce(this._sampleSpecification.result,
    ...
    readonly studyToSampleIds = remoteData<{ [studyId: string]: { [sampleId: string]: boolean } }>({
        ...
        invoke: async ()=>{
                ...
                for (const sampleSpec of this._sampleSpecification.result!) {
    ...
    readonly studyToSampleListId = remoteData<{ [studyId: string]: string }>({
        ...
        invoke: async ()=>{
            return this._sampleSpecification.result!.reduce((map, next) => {
                if (next.sampleListId) {
                    map[next.studyId] = next.sampleListId;
                }
                return map;
    ...
    readonly _sampleSpecification = remoteData({
       invoke: async ()=>{
           ...
           if (!this.sampleListCategory) {
               return this.samplesSpecification
           } else {
               const allSampleLists = await Promise.all(this.samplesSpecification.map((spec)=>{
               ...
               const category = SampleListCategoryMap[this.sampleListCategory!];
